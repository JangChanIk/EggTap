...
public class PageBar {
	...
	//전체레코드 갯수
	private int totalRecord;//list.size():47rows
	//페이지당 레코드 수
	private int numPerPage;//페이지당 10개씩
	...
	public PageBar(int numPerPage, int totalRecord, int nowPage, String pagePath) {
		...
		this.totalPage = 
				(int)Math.ceil((double)this.totalRecord/this.numPerPage); //47.0/10 => 4.7 => 5 page
		this.totalBlock= 
				(int)Math.ceil((double)this.totalPage/this.pagePerBlock); //5.0/2 => 2.5 => 3 block
